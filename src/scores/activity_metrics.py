...
STEEPNESS_TYPES = {"road_bike", "gravel_bike", "mountain_bike"}
...
def _hr_bonus(zones4plus_minutes, points_per_interval, interval_minutes):
    """Points from time spent in zones4+"""
...
def activity_load_score(
    ...
    max_hr=MAX_HR,
    weight_volume_kg=None,        # stored in kg, converted to lbs
    ...
    """Compute activity load score using my bespoke point-based system. Scores are uncapped so 
    hard days genuinely score higher."""
    ...
    # Walking and Hiking (only specifically recorded activities rather than Garmin steps)
    ...
    if activity_type == "strength":
        base     = 30 * dur_hrs
        hr_pts   = zones4plus_minutes or 0   # 1 pt per minute in zone4+
